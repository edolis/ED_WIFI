 ...
 */
    ...
    const WiFiService::APCredential* WiFiService::APCredentialManager::activeSSIDs[maxTrackedSSIDs + 1] = {
        nullptr};
    ...
    */
        ...
        if (event_base == WIFI_EVENT)
        {
            ...
            case WIFI_EVENT_STA_DISCONNECTED:
                ...
                else
                {
                    ...
                    { // no alternative or no network, switches to AP mode and
...

    {
        ...
        if (ret == ESP_ERR_NVS_NO_FREE_PAGES || ret == ESP_ERR_NVS_NEW_VERSION_FOUND)
        {
            ...
            ret = nvs_flash_init(); //NOTE it is ERASING flash if it fails. backup needs to be properly ensured
        }
        ESP_ERROR_CHECK(ret);
    ...
    /*
        bool WiFiService::APCredentialManager::packCredential( //TODO manage also the parameter of connection enabled. want to be able to configure and save monitored networks as well
            CodecOperation mode, std::string& ssid, std::string& password,
            std::string& codecStr)
        {
            ...
            else
            {
                ...
                if (sep == std::string::npos || sep == 0
                 //   || sep == codecStr.size() - 1 allows empty password in case of ssid which just requires monitoring
                )
                    return false;
    ...
    */
        ...
        /*
                    ...
                    err == ESP_ERR_NVS_NEW_VERSION_FOUND)
                {
                    ESP_RETURN_ON_ERROR(err, TAG,
                                        "NVS error: %s. Manual recovery may be needed.",
                                        esp_err_to_name(err));
                ...
                while (true)
                {
                    ...
                    if (nvs_get_str(nvs_handle, ssid_key, nvs_ssid, &ssid_len) !=
                            ESP_OK ||               // reached end of storage and no matching SSID found
                        strcmp(nvs_ssid, ssid) == 0 // matching SSID
                    )
                        break; //  i set to the new entry index in case it needs adding or
                               //  matching

                    // Write SSID and password. if SSID already exists, it will be overwritten
                    // safely as per documentation
                    err = nvs_set_str(nvs_handle, ssid_key, ssid);
                    ESP_RETURN_ON_ERROR(err, TAG,
                                        "Could not save SSID to NVS for key %s, SSID {%s}",
                                        ssid_key, ssid);

                    err = nvs_set_str(nvs_handle, pass_key, password);
                    ESP_RETURN_ON_ERROR(err, TAG,
                                        "Could not save password to NVS for key %s, SSID {%s}",
                                        ssid_key, ssid);

                    // Commit changes
                    err = nvs_commit(nvs_handle);
                    ESP_RETURN_ON_ERROR(err, TAG, "Could not save commit SSID changes to NVS");
    ...
    esp_err_t WiFiService::APCredentialManager::loadFromNVS()
    {
        ...
        if (err != ESP_OK)
        { // logging as warning as this is a normal situation
        ...
        while (true)
        {
            ...
            if (nvs_get_str(nvs_handle, ssid_key, nvs_ssid, &ssid_len) != ESP_OK ||
                nvs_get_str(nvs_handle, pass_key, nvs_password, &pass_len) !=
                    ESP_OK)
            {
